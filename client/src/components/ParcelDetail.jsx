...
function ParcelDetail() {
      ...
      try {
        ...
        const response = await fetch(
          `https://router.project-osrm.org/route/v1/driving/${
            parcel.pickup_lon},${parcel.pickup_lat};${
            parcel.destination_lon},${parcel.destination_lat
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-xl shadow-lg mt-8">
      ...
      <h2 className="text-3xl font-bold text-blue-600 mb-6">Parcel #{parcel.id}</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="p-4 bg-blue-50 rounded-lg">
          <h3 className="text-sm font-medium text-blue-600 mb-2">Current Location</h3>
          <p className="font-medium">{parcel.present_location || 'In transit'}</p>
        </div>
      </div>
      ...
      <div className="space-y-4 border-t pt-6">
        ...
        <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
          <span className="text-sm font-medium text-blue-600">Pickup Address:</span>
          <span className="font-medium text-right max-w-[200px]">{parcel.pickup_address}</span>
        </div>

        <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
          <span className="text-sm font-medium text-blue-600">Destination:</span>
          <span className="font-medium text-right max-w-[200px]">{parcel.destination_address}</span>
        </div>
      </div>
